...
object App {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[AKRConfig]("akr", "1.x") {
      def options = Seq(
    	opt("s", "start-at", "Starts brute force at the given password") { (v: String, c: AKRConfig) => c.copy(startAt = Some(v)) },
  ...
  def startBruteForce(c: AKRConfig) = {
    ...
    val passwordGenerator = new PasswordGenerator(c.startAt, c.passwordLengthStart)
